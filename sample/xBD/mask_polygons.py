...
#    jsons = [j for j in next(walk(json_path))[2] if '_pre' in j]
    json_path = 'F:/Python_Project/Data/xBD/guatemala-volcano/labels/'
    jsons = [j for j in next(walk(json_path))[2] if '_pre' in j]
...
#        print(chip_json)
        ...
        chip_file = path.join(image_dir, chip_image_id)
#        print(chip_file)
...
#            if single_file:
            if 1:
                if 0 > 0:
                    masked_polys = mask_polygons_together_with_border(chip_size, polys, 0)
                else:
...
#    disasters = next(walk(args.input))[1]
    disasters = next(walk('F:/Python_Project/Data/xBD/guatemala-volcano/'))[1]
...
#        image_dir = path.join(args.input, disaster, 'images')
        image_dir = 'F:/Python_Project/Data/xBD/guatemala-volcano/images'
#        json_dir = path.join(args.input, disaster, 'labels')
        json_dir = 'F:/Python_Project/Data/xBD/guatemala-volcano/labels'
#        output_dir = path.join(args.input, disaster, 'masks')
        output_dir = 'F:/Python_Project/Data/xBD/guatemala-volcano/masks'
